...
def write_matrix(matrix: list[list], path: str, delimiter: str = ",", header: list[str] = None):
    with open(path, "w", encoding="utf-8") as f:
        if header is not None:
            print(delimiter.join(header), file=f)
        for inner_list in matrix:
            print(delimiter.join(str(i) for i in inner_list), file=f)
...
def read_matrix(path: str, extract_header: bool = False, encoding: str = "utf-8"):
    with open(path, encoding=encoding) as f:
        lines = f.readlines()
    matrix = []
    header = None
    for i, line in enumerate(lines):
        if extract_header and i == 0:
            header = line.split()
        else:
            matrix.append(line.split())
    return (matrix, header) if header else matrix
