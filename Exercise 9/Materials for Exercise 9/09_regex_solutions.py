...
# Regex can also match "\n" characters, so you can search for some pattern that
# starts with "# id: " and ends with "\n". In addition, assume that there can be
# arbitrary many (also 0) space " " characters following the colon of "# id:".
some_header = """
# alpha: 55
# beta: 62
# some stuff
...
# of space characters " " and >= 1 arbitrary characters "(.+)" followed by a
# line break "\n". Group 1 will return the characters in the group "(.+)".
extracted_id = re.search(r"# id: *(.+)\n", some_header).group(1)
